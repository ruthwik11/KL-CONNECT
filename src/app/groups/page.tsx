...
import { GroupWindow } from "@/components/groups/group-window";
...
export default function GroupsPage() {
  ...
  if (checkingAuth) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-surface font-[family-name:var(--font-press-start)] text-[10px] text-primary gap-4 animate-pulse uppercase">
        <span>🔋 SYNCHRONIZING MULTIPLAYER ROOMS...</span>
        <div className="w-48 h-2 border-2 border-primary rounded-[2px] overflow-hidden p-0.5">
          <div className="h-full bg-primary rounded-[1px]" style={{ width: "80%" }} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen w-screen bg-surface overflow-hidden">
      ...
      {/* 2. MIDDLE DISCUSSION DIRECTORY (Fixed 80 units) */}
      ...
      {/* 3. RIGHT ACTIVE DISCUSSION CANVAS (Flex-1) */}
      <div className="h-full flex-1 min-w-0">
        <GroupWindow groupId={activeGroupId} triggerRefresh={triggerRefresh} />
      </div>
